...
def _sample_negative_indices(
    ...
    for batch_idx in range(batch_size):
        ...
        print('mask_time_indices[batch_idx]:', mask_time_indices[batch_idx])
        print('mask_time_indices[batch_idx]:', mask_time_indices[batch_idx].sum())
...
class Wav2Vec2SdpaAttention(Wav2Vec2Attention):
    ...
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "Sign2VecModel is using Wav2Vec2SdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
